...
static void DisplayShow_Once(void)
{
  ...
  TFT_LCD.TFT_DrawRectangle(0, 0, LCD_W-1, LCD_H-1, Color_WHITE);
  TFT_LCD.TFT_DrawRectangle(0, 108, LCD_W-1, 127, Color_GRAY);

  // 分区分隔线：增强层次感与可读性。
  TFT_LCD.TFT_DrawLine(0, 30, LCD_W-1, 30, Color_GRAY);
  TFT_LCD.TFT_DrawLine(0, 62, LCD_W-1, 62, Color_GRAY);
  TFT_LCD.TFT_DrawLine(0, 78, LCD_W-1, 78, Color_GRAY);
  TFT_LCD.TFT_DrawLine(0, 106, LCD_W-1, 106, Color_GRAY);
  TFT_LCD.TFT_DrawLine(76, 63, 76, 77, Color_GRAY);
...
static void DisplayShow_Device(void)
{
  ...
  TFT_LCD.TFT_ShowString(80, 64, buf,
...
static void DisplayShow_Setval(void)
{
  ...
  TFT_LCD.TFT_ShowString(0, 32, buf, color_vset, Color_BLACK, ASCII_font_16, font_overlay_ON);

  snprintf(buf, sizeof(buf), "ISET:%5.3fA", Display_GetSetCurrent());
  TFT_LCD.TFT_ShowString(0, 48, buf, color_iset, Color_BLACK, ASCII_font_16, font_overlay_ON);
}
...
static void DisplayShow_Outval(void)
{
  ...
  TFT_LCD.TFT_ShowString(0, 80, buf, Color_WHITE, Color_BLACK, ASCII_font_16, font_overlay_ON);

  snprintf(buf, sizeof(buf), "P:%4.2fW T:%4.1fC", p_out, temperature);
  TFT_LCD.TFT_ShowString(0, 96, buf,
                         temp_color,
                         Color_BLACK, ASCII_font_16, font_overlay_ON);
  ...
  if(percent > 0)
  {
    TFT_LCD.TFT_FillColor(0, 108, (uint16_t)((LCD_W-1) * percent / 100), 127, row_color);
  }

  snprintf(buf, sizeof(buf), "%3d%%", percent);
  TFT_LCD.TFT_ShowString(120, 80, buf, row_color, Color_BLACK, ASCII_font_16, font_overlay_ON);

  // 设置模式下，用矩形高亮当前调节行。
  if(Function_SET.SetMenuState == Menu_SET_State)
  {
    // 先擦除两条候选高亮边框，避免切换时残留旧框。
    TFT_LCD.TFT_DrawRectangle(0, 32, LCD_W-1, 47, Color_BLACK);
    TFT_LCD.TFT_DrawRectangle(0, 48, LCD_W-1, 63, Color_BLACK);

    cursor_color = (Function_SET.SetVIState == SET_V_State) ? Color_YELLOW : Color_CYAN;
    if(Function_SET.SetVIState == SET_V_State)
    {
      TFT_LCD.TFT_DrawRectangle(0, 32, LCD_W-1, 47, cursor_color);
    }
    else
    {
      TFT_LCD.TFT_DrawRectangle(0, 48, LCD_W-1, 63, cursor_color);
    }
  }
}
